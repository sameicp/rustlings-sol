...
fn num_valid_int(arr: &[i16]) -> usize {
    arr.iter().filter(|&a| *a >= 0 && *a < 256).count()
}
...
impl TryFrom<(i16, i16, i16)> for Color {
    ...
    fn try_from(tuple: (i16, i16, i16)) -> Result<Self, Self::Error> {
        let result = num_valid_int(&[tuple.0, tuple.1, tuple.2]);
        if result != 3 {
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        let result = num_valid_int(slice);
        if slice.len() != 3 {
